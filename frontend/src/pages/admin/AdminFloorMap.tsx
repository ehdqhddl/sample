...
const EMPTY: Omit<FloorMap, 'id'> = {
  floorNumber: '', floorNameKo: '', floorNameEn: '', floorNameZh: '', floorNameJa: '',
  descriptionKo: '', descriptionEn: '', descriptionZh: '', descriptionJa: '',
  imageUrl: null, displayOrder: 0, active: true
}

export default function AdminFloorMap() {
  const { t } = useTranslation()
  const qc = useQueryClient()
  const [modal, setModal] = useState<{ open: boolean; editing: FloorMap | null }>({ open: false, editing: null })
  const [form, setForm] = useState(EMPTY)
  const [lang, setLang] = useState<'Ko' | 'En' | 'Zh' | 'Ja'>('Ko')

  const { data: floorsRes, isLoading } = useQuery({ queryKey: ['admin', 'floors'], queryFn: adminApi.getAllFloorMaps })
  const floors: FloorMap[] = floorsRes?.data?.data || []

  const createMutation = useMutation({
    mutationFn: (data: typeof EMPTY) => adminApi.createFloorMap(data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin', 'floors'] }); closeModal(); toast.success('저장되었습니다') },
    onError: () => toast.error('오류가 발생했습니다'),
  })
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: typeof EMPTY }) => adminApi.updateFloorMap(id, data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin', 'floors'] }); closeModal(); toast.success('수정되었습니다') },
    onError: () => toast.error('오류가 발생했습니다'),
  })
  const deleteMutation = useMutation({
    mutationFn: (id: number) => adminApi.deleteFloorMap(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin', 'floors'] }); toast.success('삭제되었습니다') },
    onError: () => toast.error('오류가 발생했습니다'),
  })

  const openCreate = () => { setForm(EMPTY); setModal({ open: true, editing: null }) }
  const openEdit = (item: FloorMap) => {
    const { id, ...rest } = item
    setForm(rest)
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-gray-600 font-medium">층명 (KO)</th>
                <th className="text-left px-4 py-3 text-gray-600 font-medium hidden md:table-cell">설명</th>
                <th className="text-left px-4 py-3 text-gray-600 font-medium">순서</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {floors.map(item => (
                <tr key={item.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-semibold text-gold-700">{item.floorNumber}</td>
                  <td className="px-4 py-3 text-gray-700">{item.floorNameKo}</td>
                  <td className="px-4 py-3 text-gray-500 max-w-xs truncate hidden md:table-cell">{item.descriptionKo}</td>
                  <td className="px-4 py-3 text-gray-500">{item.displayOrder}</td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-lg shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b">
              <h3 className="font-semibold">{modal.editing ? t('admin.floorMap.edit') : t('admin.floorMap.add')}</h3>
              <button onClick={closeModal} className="text-gray-400 hover:text-gray-600"><X size={20} /></button>
            </div>
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-600 mb-1">층 번호</label>
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">층명 ({lang})</label>
                <input className="w-full px-3 py-2 border border-gray-300 focus:outline-none focus:border-gold-500 text-sm"
                  value={(form as any)[`floorName${lang}`]}
                  onChange={e => setForm({ ...form, [`floorName${lang}`]: e.target.value })} />
              </div>
              <div>
              ...
              <div>
                ...
                <input className="w-full px-3 py-2 border border-gray-300 focus:outline-none focus:border-gold-500 text-sm"
                  value={form.imageUrl || ''} onChange={e => setForm({ ...form, imageUrl: e.target.value || null })} />
              </div>
